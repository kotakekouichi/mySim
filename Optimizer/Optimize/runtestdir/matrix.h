...
class AbstMatrix
{
  ...
  public:
    ...
    double *preNonZeroColIndex;
    double *preNonZeroRowIndex;
    double *nextNonZeroColIndex;
    double *nextNonZeroRowIndex;

    map<int, double> compmap;
    map<int, double> compmaptrace;
    ...
    void InitMatrix(int nRow, int nCol)
    {
      ...
      preNonZeroColIndex = new double[nrow*ncol];
      preNonZeroRowIndex = new double[nrow*ncol];
      nextNonZeroColIndex = new double[nrow*ncol];
      nextNonZeroRowIndex = new double[nrow*ncol];
...
//	    this->a[i][j] = 0.0;
    ...
    inline void setComponent(int i , int j, double val)
    {
      int index = IDX(i, j);
      this->a2[index] = val;
      if(val != 0.0) this->compmap[index] = val;
    }
    inline double getComponent(int i, int j ) 
    {
      //return this->a2[i + j * nrow];
      int index = IDX(i, j);
      return this->compmap.find(index) != this->compmap.end() ? this->compmap[index] : 0.0;
    }
...
class AbstVector
{
  ...
  public:
    ...
    void Lforwardsubs(AbstVector &bvec)
    {
      ...
      {
	this->c[i] /= this->L->getComponent(i, i);

	//for(int j = L->getFirstRowIndex(i+1, i); j != -1; j = L->getNextNonZeroRowIndex(j, i))
	for(int j = i + 1; j < Rank; ++j)
	{
	  this->c[j] -= this->getComponent(i) * this->L->getComponent(j, i);
	}
    ...
    void Ubackwardsubs()
    {
      ...
      {
	this->c[i] /= this->U->getComponent(i, i);

	//for(int j = U->getLastRowIndex(i - 1, i); j != -1; j = U->getPreNonZeroRowIndex(j, i))
	for(int j = i - 1; j >= 0; --j)
	{
	  this->c[j] -= this->getComponent(i) * this->U->getComponent(j, i);
	}
      }
    }
    ...
    void LUdecompositionReusing(AbstMatrix *A, AbstVector &bvec)
    {
      ...
      {
	int icol = oldIndex[i];
	if( this->c[icol] == 0) continue;
	
	double dbl = this->c[icol];
...
/*
	   if(this->c[j] == 0.0 && this->cmap.find(j) != this->cmap.end())
	     this->cmap.erase(this->cmap.find(j));
	   else if(this->c[j] != 0.0)
...
/*
	if(this->c[icol] == 0.0 && this->cmap.find(icol) != this->cmap.end())
	  this->cmap.erase(this->cmap.find(icol));
	else if(this->c[icol] != 0.0)
	  this->cmap[icol] = this->c[icol];
	  */
	...
	//for(int j = 0; j < Rank; ++j) 
	  //dbl += (j == icol ? oldx[i].c[j] - 1. : oldx[i].c[j] ) * bvec.c[j] / oldx[i].c[icol]; 
#if 1	
...
#endif
	//for(int j = 0; j < Rank; ++j)
	  //bvec.c[j] = bvec.c[j] - (j == icol ? 1 : 0) * dbl; 
	bvec.c[icol] = bvec.c[icol] - dbl;
	bvec.cmap[icol] = bvec.cmap[icol] - dbl;
	if(bvec.cmap.find(icol) != bvec.cmap.end())
	  if(bvec.cmap[icol] == 0.0) 
	  bvec.cmap.erase(bvec.cmap.find(icol));
      }
