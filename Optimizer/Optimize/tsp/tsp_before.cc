...
void MininumSpanningTree(NodeClass *Nodes, double &distmin);
...
int main()
{
  ...
  MininumSpanningTree(Nodes, distmin);
...
void BranchAndBound(NodeClass *Nodes, double &distmin)
{
  ...
  for(int i = 0; i < N; ++i)
  {
    ...
    nodenum = 1;
    dist = 0.0;
    recursive(Nodes, nodenum, inode, distmin, dist, FirstNode);
...
void recursive(NodeClass *Nodes, int nodenum, int *inode, double &distmin, double &dist, NodeClass *FirstNode)
{
  ...
  {
    NodeClass *Node = &Nodes[i];
    if(Node->getPassingFlag()) continue;
    Node->setPassingFlag(true);
...
void MininumSpanningTree(NodeClass *Nodes, double &distmin)
{
  ...
  for(map<double, PathClass>::iterator ite = PathMap.begin(); ite != PathMap.end(); ++ite)
  {
    ...
    else 
    {
      ...
      for(NodeClass *node = Forest1->getFirstNode(); node != NULL; node = node->getNextNode())
      {
